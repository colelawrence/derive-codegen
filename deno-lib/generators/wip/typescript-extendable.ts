// WIP: Generator builder
import { Code, gen } from "../../mod.ts";

declare global {
  // deno-lint-ignore no-empty-interface
  interface TypeScriptCodegenSettings {}
  interface TypeScriptCodegenHook {
    start(input: gen.Input, output: gen.Output): void;
  }
}

export function buildGenerator(): TypeScriptGeneratorBuilder<{
  base: TypeScriptCodegenSettings["base"];
}> {
  function makeGenerator(
    extensions: TypeScriptCodegenExtension<keyof TypeScriptCodegenSettings>[],
  ): TypeScriptGeneratorBuilder<{}> {
    return {
      extensions,
      extend(ext) {
        return makeGenerator([...extensions, ext]);
      },
      generate(input, settings) {
        ...
        // should this actually require the calling code to iterate over hooks?
        function emit(hookName: string, ...args: unknown[]) {
          const found = hooks.get(hookName);
          if (!found?.length) return;
          for (const hookFn of found) {
            hookFn(...args);
          }
        }
        for (const ext of extensions) {
          const extHooks = ext.hooks({
          ...
          for (const key in extHooks) {
            if (hooks.has(key)) {
              // @ts-ignore internal
              hooks.get(key)!.push(extHooks[key]);
            } else {
              // perf todo: should set direct value when only one value, and set array when multiple
              // @ts-ignore internal
              hooks.set(key, [extHooks[key]]);
            }
          }
        }
        emit("start", input, output);
        return output;
      },
    };
  }
...
interface TypeScriptCodegenExtension<Name extends keyof TypeScriptCodegenSettings> {
  name: Name;
  hooks(context: {
    settings: TypeScriptCodegenSettings[Name];
    allSettings: Partial<TypeScriptCodegenSettings>;
    emit<H extends keyof TypeScriptCodegenHook>(hook: H, ...args: Parameters<TypeScriptCodegenHook[H]>): void;
  }): Partial<TypeScriptCodegenHook>;
  defaults: TypeScriptCodegenSettings[Name];
}

interface TypeScriptGeneratorBuilder<Settings extends Partial<TypeScriptCodegenSettings>> {
  extensions: TypeScriptCodegenExtension<keyof TypeScriptCodegenSettings>[];
  extend<Extension extends TypeScriptCodegenExtension<keyof TypeScriptCodegenSettings>>(
    extension: Extension,
  ): TypeScriptGeneratorBuilder<Settings & Extension["defaults"]>;
  generate(input: gen.Input, settings: Partial<Settings>): gen.Output;
}
...
declare global {
  ...
  interface TypeScriptCodegenHook {
    declaration(params: {
      ...
      docs: string[];
      output: { code: Code; skipGenerating: boolean };
    }): void;
  }
}
defineTypeScriptExtension({
  name: "base",
  defaults: {
    includeLocationsRelativeTo: undefined,
  },
  hooks(ctx) {
    const args = ctx.settings;
    return {
      start(input, output) {
        ...
        for (const decl of input.declarations) {
          ...
          const output = {
            code: generated,
            skipGenerating: false,
          };
          ctx.emit("declaration", {
            decl,
            docs,
            output,
          });

          if (output.skipGenerating) continue;
                ...
                gen.VariantFormat.match(variant.variant_format, {
                  NewType(format) {
                    ...
                    $nsMatchIfObjs.add`if (${namedStr(
                      variant,
                    )} in input) return to.${variantNameField}(input[${namedStr(variant)}]);`;
                  },
                  Tuple(formats) {
        ...
        // TODO: Move these to a "scalars" extension
        /* 
        hook: declaration
          if (decl.codegen_flags?.scalar) {
            ...
            if (args.importScalarsFrom) {
              ...
              output.code.lines.push(...docs);
              output.code.add`export type ${scalarIdent} = ${importAsIdent};`;
            }
            output.code.lines.push(...docs);
            output.code.add`export function ${scalarIdent}(value: ${scalarIdent}): ${scalarIdent} {`;
            output.code.ad1`return value;`;
            output.code.add`}`;
        ...
        if (args.importScalarsFrom) {
          if (scalarIdents.length === 0) {
            ...
            generated.lines.unshift(
              `import { ${scalarIdents.map((a) => `${a.import} as ${a.as}`).join(", ")} } from "${
                args.importScalarsFrom
              }";`,
            );
          }
        }
...
export function defineTypeScriptExtension<Name extends keyof TypeScriptCodegenSettings>(
  extension: TypeScriptCodegenExtension<Name>,
): TypeScriptCodegenExtension<Name> {
  return extension;
}

export function splitByFlattened<T extends gen.Attrs>(items: T[]) {
  const flattened: T[] = [];
  const fields: T[] = [];
